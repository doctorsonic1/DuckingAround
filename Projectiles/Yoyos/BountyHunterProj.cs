...
using System;
using DuckingAround.Projectiles.Yoyos.Orbiting.Bountyhunter;

namespace DuckingAround.Projectiles.Yoyos
{
	public class BountyHunterProj: ModProjectile
	{
		...
		public override void AI()
		{
			if (!yoyosSpawned && projectile.owner == Main.myPlayer)
			{
				...
				for (int index = 0; index < num; ++index)
				{
					...
					if (index == 0)
					{
						DuckingProjectile.NewProjectileDirectSafe(projectile.Center, Vector2.Zero, ModContent.ProjectileType<BountyOrbiting1>(), projectile.damage, projectile.knockBack, projectile.owner, 5f, ai1).localAI[0] = (float)projectile.whoAmI;
					}
					if (index == 1)
					{
						DuckingProjectile.NewProjectileDirectSafe(projectile.Center, Vector2.Zero, ModContent.ProjectileType<BountyOrbiting2>(), projectile.damage, projectile.knockBack, projectile.owner, 5f, ai1).localAI[0] = (float)projectile.whoAmI;
					}
					if (index == 2)
					{
						DuckingProjectile.NewProjectileDirectSafe(projectile.Center, Vector2.Zero, ModContent.ProjectileType<BountyOrbiting3>(), projectile.damage, projectile.knockBack, projectile.owner, 5f, ai1).localAI[0] = (float)projectile.whoAmI;
					}
				}
